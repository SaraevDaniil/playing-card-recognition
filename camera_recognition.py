...
# Transforms as in the model creation
transform = transforms.Compose([
    transforms.Resize((128, 128)),
    transforms.ToTensor(),
])

# Keep last 5 frames of detected cards
history_length = 5 # tracking state
next_id = 0
card_histories = {}
prev_cards = {}
...
while True:
    ...
    detected_cards = []
    current_cards = {}
    existing_centers = {cid: (x + w // 2, y + h // 2) for cid, (x, y, w, h) in prev_cards.items()}
    ...
    for bbox, cls, conf in detected_cards:
        ...
        text = f"{cls}: {conf:.1f}%"
        cv2.putText(frame, text, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
